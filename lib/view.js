...
const SORTS = {
  ...
  // MirCloud: grouped by fleet category in a fixed order, alphabetical inside
  // each group (lib/categories.js). One key carries all three levels.
  category: {
    label: 'category',
    sort: [{ field: { token: 'cat_key' }, order: 'asc' }],
  },
};
...
// MirCloud: where the fleet's lane definitions are readable, the category view
// is what a fresh start gets.
function defaultMode() {
  return require('./categories').available() ? 'category' : DEFAULT_MODE;
}
...
function mode() {
  try {
    ...
    return value in SORTS ? value : defaultMode();
  } catch {
    return defaultMode();
  }
}
...
function resolve(current, { op, set } = {}) {
  ...
  // MirCloud: category -> grouped -> recent -> off -> category.
  if (op === 'cycle')
    return current === 'category' ? 'grouped' : current === 'grouped' ? 'recent' : current === 'recent' ? null : 'category';
  if (set === 'off') return null;
  if (set === 'grouped' || set === 'recent' || set === 'category') return set;
  return current;
}
