...
const SPACE_TOKENS = [
  ...
  'space_label',
  // MirCloud: the name and the description, split off the label for two rows.
  'space_name',
  'space_desc',
];
...
function spaceLogoTokens(agents) {
  ...
  for (const a of agents) {
    ...
    // with.
    // MirCloud: the mark alone. The Spaces row is ONE line now, logo then the
    // workspace name, and the vendor's word beside the mark only truncated the name.
    const label = logo ?? a.name;
...
function writeSpaceState(source, workspaceId, tokenName, glyph, logoTokens = {}, label = null) {
  ...
  // MirCloud: "<lane> · <description>" -> a name row and an indented description row.
  const sep = ' \u00b7 ';
  const text = label ?? '';
  const at = text.indexOf(sep);
  tokens.space_name = at < 0 ? text || null : text.slice(0, at);
  tokens.space_desc = at < 0 ? null : `\u200b  ${text.slice(at + sep.length)}`;
