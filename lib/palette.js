...
// MirCloud: every colour below the vendor table matches the MirCloud webapp's
// tokens (inventory/app/src/styles/tokens.css), dark and light, so the sidebar and
// the app read as one product. Selected row = the webapp's --line.
const chrome = {
  light: { active_row_bg: '#d9dde6' },
  dark: { active_row_bg: '#32343b' },
};
...
const brand = {
  ...
  pi: '#c084fc', // MirCloud: pi signs in black; the webapp's violet (cyan is the brand, headers wear it)
  other: '#ffb02e', // MirCloud: the webapp's --warn
};
...
const stateByVariant = {
  light: {
    // MirCloud: webapp light tokens.
    done: '#0a7d49', // --st-spare
    blocked: '#c21d3f', // --alert
    blockedDim: '#d9677c', // the blink's off-beat: --alert lightened, still over the 3:1 floor
    none: '#5d616b', // --dim
    idleFresh: '#0f1116', // --text
    idleNormal: '#3d414b', // --muted
    idleStale: '#5d616b', // --dim
    idle: '#3d414b',
    subtle: '#0e8a7c', // --brand: category headers
  },
  upstreamLight: {
    idleFresh: '#416c4f',
    idleNormal: '#6b6259',
  ...
  dark: {
    // MirCloud: webapp dark tokens.
    done: '#00ffa0', // --st-spare / mint
    blocked: '#ed1944', // --alert
    blockedDim: '#bd3752', // the blink's off-beat: --alert darkened, still over the 3:1 floor
    none: '#707070', // --dim
    idleFresh: '#e9ebf1', // --text
    idleNormal: '#b3b6c3', // --muted
    idleStale: '#8b8e9c', // between --muted and --dim: --dim alone read too faint for a whole line
    idle: '#b3b6c3',
    subtle: '#08f5da', // --brand: category headers
  },
  upstreamDark: {
    idleFresh: '#95bba2',
    idleNormal: '#a99e92',
