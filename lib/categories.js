...
//
//   managers  deployers  operators  builders  testers  misc  reports
//
...
const ORDER = ['managers', 'deployers', 'operators', 'builders', 'testers', 'misc', 'reports'];
// MirCloud: groups that hold no lanes. A workspace labelled "<group> · <row name>" joins that
// group in the fixed order, its row named by the rest of the label (fleet.yaml display_panes,
// started by fleet/bin/display-pane-up.sh). "reports" holds the 24-hr changelog first.
const DISPLAY_GROUPS = new Set(['reports']);
const BY_FUNCTION = {
...
function forLabel(workspaceId, label) {
  ...
  const at = text.indexOf(LABEL_SEP);
  const name = at >= 0 ? text.slice(at + LABEL_SEP.length).trim() : '';
  // A display group must also be in ORDER; one that is not falls through to its own group
  // rather than ranking -1 ahead of every category. An empty row name falls through too.
  if (DISPLAY_GROUPS.has(lane) && name && ORDER.includes(lane)) {
    return { key: `cat:${lane}`, label: lane, rank: ORDER.indexOf(lane), lane: name, title: name };
  }
...
function reach() {
  ...
  const fn = (lane) => (lane in (data.lanes ?? {}) ? fresh && data.lanes[lane] === true : undefined);
  // Is the lane's model answering the fleet's smoke test? undefined = no answer on file.
  fn.modelOk = (lane) => (lane in (data.model_ok ?? {}) ? fresh && data.model_ok[lane] === true : undefined);
  // The model the lane is actually running, shortened for a sidebar line.
  fn.model = (lane) => shortModel((data.models ?? {})[lane]);
  return fn;
}

// MirCloud: the model as family + version (+ "flash"), shown after the lane name:
// claude-opus-5 -> opus5, claude-sonnet-5 -> sonnet5, claude-fable-5-1 -> fable51,
// glm-5.3-flash-... -> glm53flash, deepseek-v4.1-flash-... -> deepseek41flash.
// Unknown families keep their own first word.
const FAMILY = [
  [/^claude-opus/, 'opus'],
...
function shortModel(model) {
  ...
  const hit = FAMILY.find(([re]) => re.test(model));
  const family = hit ? hit[1] : (model.match(/^[a-z]+/i)?.[0] ?? model);
  // Version digits only: quantisation and build suffixes (-fp8, -uncensored) are not the model.
  const core = model.replace(/-(?:uncensored|fp\d+|int\d+|awq|gguf)\b/gi, '');
  const digits = (core.replace(/^[a-z]+-(?:[a-z]+-)?/i, '').match(/\d+/g) ?? []).join('').slice(0, 3);
  const flash = /flash/.test(model) ? 'flash' : '';
  return `${family}${digits}${flash}`;
}

module.exports = { ORDER, available, forLabel, sortKey, parseCsv, reach, shortModel, SHORT_WIDTH, LABEL_SEP };
