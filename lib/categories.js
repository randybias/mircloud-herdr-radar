...
// MirCloud: a model in six characters or fewer, so the lane names line up:
// claude-opus-5 -> o5, claude-sonnet-5 -> s5, claude-fable-5-1 -> f51,
// glm-5.3-flash-... -> glm53f, deepseek-v4.1-flash-... -> ds41f. Unknown families
// fall back to the id's first letters and digits.
const FAMILY = [
  [/^claude-opus/, 'o'],
  [/^claude-sonnet/, 's'],
  [/^claude-haiku/, 'h'],
...
function shortModel(model) {
  ...
  const family = hit ? hit[1] : model.replace(/[^a-z]/gi, '').slice(0, 2);
  ...
  const flash = /flash/.test(model) ? 'f' : '';
  return `${family}${digits}${flash}`.slice(0, SHORT_WIDTH);
}
