...
module.exports = {
  ...
  // MirCloud: TWO, not the one the Operators asked for (2026-09-19): Herdr hangs the first
  // lane under a header at two columns itself and that cannot be changed, so any other
  // width leaves the first lane of every group out of line with the rest.
  ...
  // MirCloud: where the fleet's lane definitions live (lib/categories.js). Unset
  // means the usual places: $MIRCLOUD_LANES_CSV, ~/ops/fleet, ~/ops/mircloud/fleet.
  lanesCsv: typeof raw.lanes_csv === 'string' && raw.lanes_csv ? raw.lanes_csv : null,

  // MirCloud: force the sidebar palette to one side. `auto` follows the desktop
  // as upstream does. The desktop is the wrong authority when the TERMINAL is dark
  // and macOS is light: the plugin then paints for a light panel and a dark one
  // gets near-black logos and grey names. Measured 2026-09-19 on an Operator's
  // laptop. auto | light | dark
  appearance: ['light', 'dark'].includes(raw.appearance) ? raw.appearance : 'auto',

  // MirCloud: the fleet's per-lane reachability file (lib/categories.js reach()).
  reachFile: typeof raw.reach_file === 'string' && raw.reach_file ? raw.reach_file : null,
};
