...
class Frame {
  constructor(source) {
    ...
    this.lastReach = new Map();
    this.lastPaneName = new Map();
  ...
  async render(now) {
    ...
    const grouped =
      viewMode === 'grouped' || viewMode === 'category' || (viewMode === null && herdr.panelGrouped(now));
    // MirCloud: in `category` mode a group is a fleet category, not a workspace.
    // `groupOf` maps each pane to its group; `gEntries` are the same entries with
    // the group standing in for the workspace, which is all writeGroups and
    // groupBoundaries look at — so upstream's header, indent and spacer logic
    // draws category headers unchanged.
    const byCategory = viewMode === 'category';
    const reachOf = byCategory ? categories.reach() : null;
    const groupOf = new Map();
    if (byCategory) {
      for (const entry of entries) {
        const g = categories.forLabel(entry.workspace, workspaces.get(entry.workspace));
        if (g) groupOf.set(entry.pane, g);
      }
    }
    const regroup = (list) =>
      byCategory ? list.map((e) => (groupOf.has(e.pane) ? { ...e, workspace: groupOf.get(e.pane).key } : e)) : list;
    const groupLabels = byCategory
      ? new Map([...workspaces, ...[...groupOf.values()].map((g) => [g.key, g.label])])
      : workspaces;
    const displayEntries = regroup(this.displayOrder(entries, viewMode, keys, groupOf));
    ...
    for (const entry of entries) {
      ...
      // MirCloud: a category indents like a workspace group — Herdr hangs the head
      // lane's own line under the header for us, the others get one level of ours.
      const depth = byCategory
        ? heads.has(entry.pane)
          ? 0
          : 1
        : grouped
          ? (heads.has(entry.pane) ? 0 : 1) + (keys.nested(entry.workspace) ? 1 : 0)
          : 0;
      ...
      // MirCloud: the row's title is the lane's own label, and its sort key places
      // it in the category order.
      const group = groupOf.get(entry.pane);
      // MirCloud: agents show STATE, machines show what a lane is for. The title is
      // the short model, padded so names line up, then the lane name - no description.
      const catKey = group ? categories.sortKey(group, entry.pane) : null;
      const reachable = group && reachOf ? reachOf(group.lane) : undefined;
      const model = group && reachOf ? reachOf.model(group.lane) : null;
      const modelOk = group && reachOf ? reachOf.modelOk(group.lane) : undefined;
      // MirCloud: "<lane> (<model>)". The description is in the window title (below)
      // and in the machines pane, never here.
      const laneName = group && group.key.startsWith('cat:') ? group.lane : group?.title;
      const titleOverride = group ? (model ? `${laneName} (${model})` : laneName) : null;
      // The pane's name feeds the window title, "{hostname}: {pane}":
      // "<lane> (<model>) · <description in full>".
      if (group) {
        const label = workspaces.get(entry.workspace) ?? '';
        const at = label.indexOf(categories.LABEL_SEP);
        const desc = at < 0 ? '' : label.slice(at + categories.LABEL_SEP.length);
        const paneName = desc ? `${titleOverride}${categories.LABEL_SEP}${desc}` : titleOverride;
        if (this.lastPaneName.get(entry.pane) !== paneName && this.writable(`name:${entry.pane}`, now)) {
          jobs.push(
            herdr.renamePaneAsync(entry.pane, paneName).then((ok) => {
              if (this.settled(`name:${entry.pane}`, ok)) this.lastPaneName.set(entry.pane, paneName);
            }),
          );
        }
      }
      this.paneJobs(
        entry,
        display,
        { tabs, keys, indent, corner, spinStep, header, titleOverride, catKey, reachable, modelOk },
        now,
        deadlines,
        jobs,
      );
    ...
    await this.groupJobs(
      regroup(entries),
      displayEntries,
      viewMode,
      grouped,
      byCategory ? new Map() : wsAgents,
      groupLabels,
      byCategory ? { ...keys, parentOf: new Map(), orphanRepo: new Map() } : keys,
      now,
      deadlines,
    );
  ...
  displayOrder(entries, viewMode, { wsKeys, minuteKey, tabKeys }, groupOf = new Map()) {
    if (viewMode === 'category') {
      // The order Herdr will show: ascending `cat_key`, missing last.
      const key = (e) => (groupOf.has(e.pane) ? categories.sortKey(groupOf.get(e.pane), e.pane) : null);
  ...
  paneJobs(
    ...
    const title = titleOverride ?? state.trimGroupPrefix(entry.title, header);
    // MirCloud: when a reachability mark leads the row, the indent goes on the mark,
    // not on the logo after it — otherwise the mark sits at the margin on every row
    // but the first, and Herdr's hanging indent shifts that first row right.
    ...
    // MirCloud: in the compact row the state mark (tick, spinner, question) gets a fixed
    // two-column slot, blank when there is none - otherwise the model and name start one
    // tick further right on some rows than on others.
    if (line && titleOverride && !line.titlePrefix.trim()) line.titlePrefix = `${line.titlePrefix}\u200b  `;  // Herdr trims leading whitespace: the zero-width space holds the slot
    ...
    // MirCloud: a green circle when the lane can be woken by codev mail, a red x
    // when it cannot. Nothing at all where there is no answer (not a lane, or no
    // reachability file on this machine).
    // MirCloud: two indicators in ONE cell - codev reachable, model answering - each
    // a green circle, or an x. Any x turns the cell red and it blinks (the blocked
    // pulse's beat), because a lane nobody can wake is the thing to notice.
    let reachTokens = { reach_ok: null, reach_bad: null, reach_bad_dim: null, model: null };
    if (reachable !== undefined || modelOk !== undefined) {
      const glyph = (ok) => (ok === false ? '\u2717' : '\u25cf');
      const text = `${glyph(reachable)}${glyph(modelOk)}`;
      const failing = reachable === false || modelOk === false;
      if (!failing) reachTokens.reach_ok = text;
      else if (Math.floor(now / PULSE_MS) % 2 === 0) reachTokens.reach_bad = text;
      else reachTokens.reach_bad_dim = text;
      if (failing) deadlines.push((Math.floor(now / PULSE_MS) + 1) * PULSE_MS);
    }
    const reachPair = JSON.stringify(reachTokens);
    if (this.lastReach.get(pane) !== reachPair && this.writable(`reach:${pane}`, now)) {
      const tokens = reachTokens;
      jobs.push(
        herdr.reportMetadataAsync(pane, src, tokens).then((ok) => {
          if (this.settled(`reach:${pane}`, ok)) this.lastReach.set(pane, reachPair);
        }),
      );
    }
    ...
    const sortPair = `${sortKey}|${wsKey}|${catKey}`;
    if (this.lastSort.get(pane) !== sortPair && this.writable(`sort:${pane}`, now)) {
      jobs.push(
        herdr.reportMetadataAsync(pane, src, { sort_key: sortKey, ws_key: wsKey, tab_key: tabKey, cat_key: catKey }).then((ok) => {
