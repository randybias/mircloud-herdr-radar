...
class Frame {
  ...
  async render(now) {
    ...
    for (const entry of entries) {
      ...
      const titleOverride = group ? group.title : null;
      ...
      this.paneJobs(
        entry,
        display,
        { tabs, keys, indent, corner, spinStep, header, titleOverride, catKey, reachable, model },
        now,
        deadlines,
        jobs,
      );
  ...
  paneJobs(
    ...
    const markIndent = reachable === undefined ? '' : indent;
    const line = state.composeLine(
      entry,
      display,
      config.showTab ? (tabs.get(entry.tab) ?? '') : '',
      reachable === undefined ? indent : '',
      spinStep,
      corner,
    );
    ...
    const reachPair = reachable === undefined ? 'none' : `${reachable}|${markIndent}|${model}`;
    if (this.lastReach.get(pane) !== reachPair && this.writable(`reach:${pane}`, now)) {
      const tokens =
        reachable === undefined
          ? { reach_ok: null, reach_bad: null }
          : reachable
            ? { reach_ok: `${markIndent}\u25cf`, reach_bad: null }
            : { reach_ok: null, reach_bad: `${markIndent}\u2717` };
      // MirCloud: the model in use, on a line of its own under the lane. Herdr hangs
      // every continuation row at the same indent, so the pad lines it up under the
      // name for the first lane of a group and the rest alike.
      tokens.model = model ? `\u200b      ${model}` : null;
      jobs.push(
