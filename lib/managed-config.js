...
function chromeVariant(text) {
  // MirCloud: an explicit appearance outranks the desktop and the theme name.
...
function sidebarBlock(variant) {
  ...
  const agentRow = (vendor) =>
    [
      ...
      `[${[
        ...
        // MirCloud: can this lane be woken by codev mail — a green circle, or a red
        // x. Only one of the two is ever set, and a lane with no answer has neither,
        // so the row carries no extra separator where there is nothing to say.
        ...
        // MirCloud: codev-reachable + model-answering, one cell. Green, or red and
        // blinking (the daemon alternates the two red names) when either fails.
        cell('$reach_ok', state.done, true),
        cell('$reach_bad', state.blocked, true),
        cell('$reach_bad_dim', state.blockedDim, true),
  ...
  return [
    ...
    'row_gap = 0',
    // MirCloud: ONE row per Space — the agent's logo, then the name. The state
    // marks are dropped here on purpose: the Agents panel carries state, and a
    // second row per Space halved how many lanes fit on screen.
    // MirCloud: machines say what a lane is FOR - its name, and its description on
    // a second, indented line where it has the width to be read. No logos: the
    // agents list carries who and what state.
    `rows = [\n  [\n    ${cell('$space_name', ink)}\n  ],\n  [\n    ${cell('$space_desc', state.idle)}\n  ]\n]`,
    SIDEBAR_END,
  ].join('\n');
}
